...
public class CrewFloater : MonoBehaviour
{
    ...
    void Start()
    {
        for (int i = 0; i < 12; i++)
        {
            SpwanFloatingCrew((EPlayerColor)i, Random.Range(0f, distance));
        }
    }
    ...
    public void SpwanFloatingCrew(EPlayerColor playerColor, float dist)
    {
        if(!crewStates[(int)playerColor])
        {
            ...
            crew.GetComponent<FloatingCrew>().SetFloatingCrew(
                sprites[Random.Range(0, sprites.Count)], playerColor, direction, floatingSpeed, 
